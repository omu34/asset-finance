@php
    ...
    $latestVideos = Videos::with('file')->get();
    $latestGalleries = Gallery::with('file')->get();
    $latestNews = News::with('file')->get();
        ...
        <div class="mx-auto grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-4 lg:mx-0 lg:max-w-none lg:grid-cols-2">
            @foreach ($featuredItems as $item)
                <article data-aos="fade-up" data-aos-duration="1500"
                    ...
                    @if ($mimeType === 'video/mp4'|| $mimeType === 'video/quicktime'|| $mimeType === 'video/x-m4v'|| $mimeType === 'video/x-matroska')
                    ...
                    @elseif ($mimeType === 'image/jpeg' || $mimeType === 'image/png' ||$mimeType === 'image/jpg'|| $mimeType === 'image/gif')
                    ...
                    @if ($item instanceof \App\Models\Videos)
                        <img src="/video.svg"
                    ...
                    @elseif ($item instanceof \App\Models\News)
                        <img src="/blog.svg"
                            class="absolute text-yellow top-2/3 mb-8 left-1/2 transform -translate-x-1/2 -translate-y-28 h-12 w-12 fill-white"
                            alt="">
                    @elseif ($item instanceof \App\Models\Gallery)
                        <img src="/gallery.svg"
                            class="absolute text-yellow top-2/3 mb-8 left-1/2 transform -translate-x-1/2 -translate-y-28 h-12 w-12 fill-white"
                            alt="">
                    @endif
