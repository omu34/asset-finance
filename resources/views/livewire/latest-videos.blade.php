@php
    use Carbon\Carbon;
    $items = App\Models\Videos::orderBy('created_at', 'desc')->get();
 ...
 <div class="bg-white bg-center py-12 sm:py-16">
    <div class="mx-auto lg:px-0 px-10 max-w-5xl">
        <div class="mx-auto flex flex-col items-center sm:flex-row sm:justify-between mb-8 ">
            <h2 class="text-3xl font-bold tracking-tight lg:pb-0 pb-5 text-gray-900 sm:text-3xl">Latest Videos</h2>
            <a href="{{ url('latest-videos') }}"
        ...
        <div class="mx-auto grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-4 lg:mx-0 lg:max-w-none lg:grid-cols-4">
            ...
            {{--  {{$item->file_path}}  --}}
                <article data-aos="fade-up" data-aos-duration="1500"
                    ...
                    <a href="{{ $item->link }}{{ $item->id }}" class="absolute inset-0 z-10">
                        <span class="sr-only">
                            {{--  Go to {{ $item->title }}  --}}
                        </span>
                    </a>
...
{{--  @php
    use Carbon\Carbon;
    $items = App\Models\LatestVideos::orderBy('created_at', 'asc')->get();
@endphp
<div class="bg-white bg-center py-12 sm:py-16">
    <div class="mx-auto lg:px-0 px-10 max-w-5xl">
        <div class="mx-auto flex flex-col items-center sm:flex-row sm:justify-between mb-8 ">
        ...
        <div class="mx-auto grid max-w-2xl auto-rows-fr grid-cols-1 gap-8 sm:mt-4 lg:mx-0 lg:max-w-none lg:grid-cols-4">
            @foreach ($items as $item)
                <article data-aos="fade-up" data-aos-duration="1500"
                    ...
                    <a href="{{ $item->link }}{{ $item->id }}" class="absolute inset-0 z-10">
                        <span class="sr-only">Go to {{ $item->title }}</span>
                    </a>
                    <video autoplay muted loop class="absolute inset-0 -z-10 h-full w-full object-cover">
                        <source src="{{ Storage::url($item->file_path) }}" type="video/mp4">
                        Your browser does not support the video tag.
